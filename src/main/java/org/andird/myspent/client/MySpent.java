...
public class MySpent implements EntryPoint {
    ...
    private static final NumberFormat priceFormat = NumberFormat.getFormat("#,###.##");
    ...
    private MySpentServiceAsync mySpentPriveSvc = GWT.create(MySpentService.class);

    private void addSpending() {
        this.addSpending(this.purposeText.getValue(), this.amountText.getValue());
            ...
            try {
                ...
                final AsyncCallback<Void> callback = new AsyncCallback<Void>() {
                    ...
                    public void onSuccess(final Void result) {
                        MySpent.this.refreshSpendingList(MySpent.this.startPast.getValue(),
    ...
    private void refreshSpendingList(final String pastFirst, final String pastSecond) {
        try {
            // initialize service
            if (this.mySpentPriveSvc == null) {
                this.mySpentPriveSvc = GWT.create(MySpentService.class);
    ...
    private void setupMySpentTable() {
        ...
        this.mySpentTable.getRowFormatter().addStyleName(0, "spendingListHeader");
    ...
    private void updateTable(final SpendingClient spend, final int row) {
        ...
        this.mySpentTable.setText(row, 1, MySpent.priceFormat.format(spend.getAmount()));
        this.mySpentTable.setText(row, 2, spend.getCreateDate());
        this.mySpentTable.setWidget(row, 3, remBtn);

        // css
        this.mySpentTable.getCellFormatter().addStyleName(row, 1, "spendingListNumericColumn");
        this.mySpentTable.getCellFormatter().addStyleName(row, 3, "spendingListRemoveColumn");
    ...
    private void updateTable(final String prices) {
        ...
        // set last update
        this.timeLabel.setText(DateTimeFormat.getMediumDateTimeFormat().format(new Date())
