...
import com.appspot.myspent.client.MySpentException;
import com.appspot.myspent.client.MySpentService;
import com.appspot.myspent.shared.FieldVerifier;
...
public class MySpentImpl extends RemoteServiceServlet implements MySpentService {
	 ...
	 * 
	 * @see
	 * com.appspot.myspent.client.MySpentService#addSpending(com.appspot.myspent
	...
	public void addSpending(final String purp, final double amt) {
		if ((purp != null) && (purp.length() > 0)) {
			...
			final SpendingData spend = new SpendingData(purp, amt, userHash,
					currDate, true, currDate);
			final PersistenceManager pm = MySpentImpl.pmf
	...
	@Override
	public String getAllPrices(final int startPast, final int length)
		...
		try {
			...
			if (qRes != null) {
				...
				for (final Iterator<SpendingData> iterator = qRes.iterator(); iterator
						.hasNext();) {
					spDt = iterator.next();
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.appspot.myspent.client.MySpentService#getPrices()
	 */
	@Override
	public String getPrices(final int startPast, final int length)
			throws MySpentException {
		List<SpendingData> qRes = null;
		String res = null;
		final PersistenceManager pm = MySpentImpl.pmf.getPersistenceManager();
		try {
			...
			qRes = (List<SpendingData>) q.execute(this.getUserHash(), first,
					second);
			if (qRes != null) {
				final Gson gson = new GsonBuilder().create();
				SpendingData spDt = null;
	 ...
	 * 
	 * @see
	 * com.appspot.myspent.client.MySpentService#removeSymbol(java.lang.Long)
	 */
	@Override
	public void inactivateSpending(final int id) {
		final PersistenceManager pm = MySpentImpl.pmf.getPersistenceManager();
		try {
			final SpendingData sp = pm.getObjectById(SpendingData.class,
					Long.valueOf(id));
